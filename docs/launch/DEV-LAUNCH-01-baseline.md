...
- `.env.local` (gitignored, never committed) was updated to point `SHOPIFY_STORE_DOMAIN` at the QA store; `SHOPIFY_STOREFRONT_ACCESS_TOKEN` was left unchanged per explicit user instruction. That token 401s against the QA store (see finding 4) — a real QA-issued token is still pending from the manager.
