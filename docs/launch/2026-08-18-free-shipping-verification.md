...
(full results reviewed file-by-file; summary below). **No bypass found** —
every hit is one of:

**(a) Safe — reads a pre-computed `shippingDisplay`, never the raw metafield:**
...
- `lib/shipping-resolver/attach.ts` (`attachCardShippingDisplay`) — `gateFreeShippingClaim(resolveCardShippingDisplay(product.id), product.freeShipping)`.
- `lib/shipping-resolver/cart.ts` (`attachCartShippingDisplay`) — same gate, keyed per cart line's own product/variant.
- `app/product/[slug]/page.tsx:110` and `app/category/[slug]/[product]/page.tsx:318` — `gateFreeShippingClaims(resolveVariantsForProduct(product.id), product.freeShipping)` for the PDP's per-variant map.
- `app/search/page.tsx:133`, `app/page.tsx:81,85`, `app/partners/[partner-slug]/page.tsx:37`, `components/category/CategoryResults.tsx:115` — all call `attachCardShippingDisplay(...)` before handing products to card components.
- `app/product/[slug]/page.tsx:116-117` — related/complementary ("You May Also Like"/"You May Also Need") products are run through `attachCardShippingDisplay` before reaching `ProductView`, same as every other card grid (this is the Task 4/DEV-SHIP-04 fix already committed — previously these carried no `shippingDisplay` at all, so they never showed a badge even when the product qualified; now they show the correctly-gated one).
